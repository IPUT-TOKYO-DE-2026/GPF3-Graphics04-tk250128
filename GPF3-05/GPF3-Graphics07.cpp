...
void FrameBufferEmulator::drawUser(unsigned char* buff, int mode, int keyLevel, int keyTrigger)// 描画処理（毎フレーム呼び出される）
{
	unsigned char  color[3] = { 10, 255, 0 }; // B, G, R
	if (keyTrigger == SDLK_UP)
	{ // 上矢印キーが押されたら
		radius++;  // 半径を大きくする
	}
